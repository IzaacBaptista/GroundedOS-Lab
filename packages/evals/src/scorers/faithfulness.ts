...
function buildFaithfulnessReason(score: number, details: any): string {
  if (score >= 0.8) {
    return 'Answer appears well-grounded in retrieved context';
  }
  if (score >= 0.5) {
    const factors = [];
    if (details.overlapRatio < 0.4) factors.push('low token overlap');
    if (!details.groundingSignalsDetected) factors.push('missing grounding signals');
    if (details.ungroundedClaimsDetected) factors.push('ungrounded claims detected');
