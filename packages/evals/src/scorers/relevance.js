...
export class RelevanceEvaluator {
    ...
    async evaluate(input) {
        ...
        const overlap = questionTokens.filter((token) => answerTokens.includes(token) && !this.isStopword(token)).length;
...
function buildRelevanceReason(score, details) {
    if (score >= 0.8) {
        return 'Answer directly and comprehensively addresses the question';
    }
    if (score >= 0.5) {
        const factors = [];
        if (details.overlapRatio < 0.3)
            factors.push('limited keyword overlap');
        if (details.lengthScore < 0.8)
            factors.push('unusual length');
        if (details.evasionDetected)
            factors.push('possible evasion');
