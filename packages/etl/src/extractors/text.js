...
export class TextExtractor {
    ...
    async extract(input) {
        if (input.type !== "text") {
            throw new Error(`[${EXTRACTOR_NAME}] Unsupported modality "${input.type}". TextExtractor only handles "text".`);
        }
    ...
    async _readContent(input) {
        ...
        throw new Error(`[${EXTRACTOR_NAME}] Either 'content' or 'filePath' must be provided for modality "text".`);
    }
    _splitSections(fullText) {
