...
export async function ingest(input) {
    const extractor = EXTRACTOR_REGISTRY.find((e) => e.supportedModalities.includes(input.type));
    if (!extractor) {
        throw new Error(`[dispatcher] No extractor registered for modality "${input.type}". ` +
