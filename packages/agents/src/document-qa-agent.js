 ...
 */
function createRetrievalTool(ragServiceGetterFn) {
    return {
        name: 'retrieve-from-index',
        description: 'Retrieves top-K relevant chunks from a persisted document index based on query similarity.',
        inputSchema: {
            type: 'object',
            properties: {
 ...
 */
function createSummarizationTool() {
    return {
        name: 'summarize-with-context',
        description: 'Generates a grounded answer based on retrieved context chunks and the original question.',
        inputSchema: {
            type: 'object',
            properties: {
...
export class DocumentQAAgent extends BaseAgent {
    ragServiceGetterFn = null;
    constructor(config) {
        super(config?.id || 'document-qa-agent', config?.name || 'Document QA Agent', config?.description || 'Answers questions grounded in indexed documents.', 'Answer user questions by retrieving relevant document chunks and generating grounded responses with source attribution.');
     ...
     */
    async reasoningStep(input, context) {
        ...
        if (step === 1) {
            ...
            if (lastToolCall?.status === 'success' && lastToolCall.output) {
                const output = lastToolCall.output;
                return {
...
//# sourceMappingURL=document-qa-agent.js.map