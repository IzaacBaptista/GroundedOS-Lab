...
function simulateDraft(baseAnswer: string, question: string): string {
  if (!baseAnswer.trim()) {
...
function simulateRefinement(draft: string, context: string): string {
  const contextHint = context.trim().length > 0 ? "Grounded with retrieved context." : "Limited context available.";
  return `${draft}\n\n${contextHint}`;
}

function verifyGrounding(answer: string, context: string): boolean {
  if (!context.trim()) {
    return false;
  }
