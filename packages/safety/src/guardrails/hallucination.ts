...
export class HallucinationGuardrail implements Guardrail {
  readonly name = 'hallucination-detector';
  readonly riskType = 'hallucination' as const;

  async check(input: GuardrailInput | HallucinationCheckInput): Promise<GuardrailResult> {
    ...
    const hallucInput = input as HallucinationCheckInput;
